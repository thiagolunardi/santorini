...
public class GameTests
{
    ...
    public void GetAvailableMoves_should_only_include_builds_adjacent_to_moveto()
    {
        ...
        foreach (var move in availableMoves)
        {
            ...
            buildDx.Should().BeLessThanOrEqualTo(1,
                $"build ({move.BuildAt.X},{move.BuildAt.Y}) must be adjacent to moveTo ({move.MoveTo.X},{move.MoveTo.Y})");
            buildDy.Should().BeLessThanOrEqualTo(1,
                $"build ({move.BuildAt.X},{move.BuildAt.Y}) must be adjacent to moveTo ({move.MoveTo.X},{move.MoveTo.Y})");
        }
    }
    ...
    public void GetAvailableMoves_all_moves_are_executable()
    {
        ...
        moves.Should().AllSatisfy(m =>
        {
            ...
            testGame.TryMoveWorker(m).Should()
                .BeTrue(
                    $"move {m.WorkerNumber} to ({m.MoveTo.X},{m.MoveTo.Y}) build ({m.BuildAt.X},{m.BuildAt.Y}) should be executable");
        });
    }
